<script>(function () {
  ...
  const ENCRYPTION_KEY = "+1wv7SIgQoWmjAu5QgFf39vQyJYlP7dg9LG+k7En/zI=";
  const ENCRYPTION_IV = "tKeo9lMPp1E8F5rS";
  ...
  async function getVisitorSessionToken() {
    try {
      ...
      const response = await fetch('https://consentbit-worker.consentapp.workers.dev/api/visitor-token', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          const retryResponse = await fetch('https://consentbit-worker.consentapp.workers.dev/api/visitor-token', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
  ...
  async function fetchCookieExpirationDays() {
    ...
    try {
      ...
      const apiUrl = `https://consentbit-worker.consentapp.workers.dev/api/app-data?siteName=${encodeURIComponent(siteName)}`;
  ...
  async function saveConsentStateToServer(preferences, cookieDays, includeUserAgent) {
    try {
      ...
      const response = await fetch("https://consentbit-worker.consentapp.workers.dev/api/v2/cmp/consent", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${sessionToken}`,
  ...
  async function checkPublishingStatus() {
    try {
      ...
      const apiUrl = `https://consentbit-worker.consentapp.workers.dev/api/site/subscription-status?siteDomain=${encodeURIComponent(siteDomain)}`;
      const response = await fetch(apiUrl, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${sessionToken}`,
  ...
  async function scanAndSendHeadScriptsIfChanged(sessionToken) {
    ...
    try {
      ...
      const apiUrl = `https://consentbit-worker.consentapp.workers.dev/api/v2/cmp/head-scripts?siteName=${encodeURIComponent(siteName)}`;
